...
copy /Y utils\dev_sidecar\target\release\VRCT_codexCLI-sidecar-x86_64-pc-windows-msvc.exe src-tauri\bin\VRCT_codexCLI-sidecar-x86_64-pc-windows-msvc.exe
if errorlevel 1 exit /b 1
REM tauri dev does not always re-copy the externalBin from src-tauri\bin\ into
REM its cached build output (src-tauri\target\debug\VRCT_codexCLI-sidecar.exe /
REM target\release\VRCT_codexCLI-sidecar.exe) when the Rust side has no source changes.
...
if exist src-tauri\target\debug\VRCT_codexCLI-sidecar.exe (
    copy /Y utils\dev_sidecar\target\release\VRCT_codexCLI-sidecar-x86_64-pc-windows-msvc.exe src-tauri\target\debug\VRCT_codexCLI-sidecar.exe
    if errorlevel 1 exit /b 1
)
if exist src-tauri\target\release\VRCT_codexCLI-sidecar.exe (
    copy /Y utils\dev_sidecar\target\release\VRCT_codexCLI-sidecar-x86_64-pc-windows-msvc.exe src-tauri\target\release\VRCT_codexCLI-sidecar.exe
    if errorlevel 1 exit /b 1
)
...
if errorlevel 1 exit /b 1
echo dev sidecar wrapper installed at src-tauri\bin\VRCT_codexCLI-sidecar-x86_64-pc-windows-msvc.exe
endlocal
