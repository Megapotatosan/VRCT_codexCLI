    ...
    const errorHandling_Backend = ({error_code, message, data, endpoint, result}) => {
        switch (error_code) {
            ...
            case "GENERAL_UNKNOWN":
                ...
                showNotification_Error(message, { category_id: error_code });
                showNotification_Error(`An error occurred. Please contact the developers and restart VRCT_codexCLI. Error: ${error_code} - ${message || JSON.stringify(result)}`, { hide_duration: null, category_id: error_code });
                return;
